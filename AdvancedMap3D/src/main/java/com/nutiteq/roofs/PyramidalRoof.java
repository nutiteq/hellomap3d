...
public class PyramidalRoof extends Roof {
  ...
  public PyramidalRoof(float roofHeight, boolean alongLongSide) {
  ...
  public void calculateRoof(Geometry minRectangle) {
    ...
    e1Point = c2Point.getSubtracted2D(c1Point);
    e3Point = c4Point.getSubtracted2D(c3Point);
    ...
    lines = new LineString[4];
    Coordinate[] lineCoords = new Coordinate[2];
    lineCoords[0] = new Coordinate(rMidPoint.x, rMidPoint.y);
    ...
    // Hipped roof for a rectangle has 4 parts, calculate the first triangle
    polygons = new Polygon[4];
    Coordinate[] polyCoords = new Coordinate[4];
    polyCoords[0] = new Coordinate(c1Point.x, c1Point.y);
    polyCoords[1] = new Coordinate(rMidPoint.x, rMidPoint.y);
    ...
    polyCoords[0] = new Coordinate(rMidPoint.x, rMidPoint.y);
  ...
  public double calculateRoofPointHeight(double x, double y) {
    ...
    if (tri1.contains(p)) {
      double distanceFromEdge = Math.abs((x - c1Point.x) * (e1Point.y) - 
          (y - c1Point.y) * (e1Point.x)) / e1Length;
      return  (distanceFromEdge / triLength) * roofHeight;
    } else if (tri2.contains(p)) {
      double distanceFromEdge = Math.abs((x - c3Point.x) * (e3Point.y) - 
          (y - c3Point.y) * (e3Point.x)) / e1Length;
      return  (distanceFromEdge / triLength) * roofHeight;
    }
