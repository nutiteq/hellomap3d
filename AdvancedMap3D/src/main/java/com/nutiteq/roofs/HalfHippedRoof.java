...
public class HalfHippedRoof extends Roof {
  //Ratio between the desired ridge length and the length of the whole bounding rectangle
  private static final float HIP_RATIO = 0.55f;
  
  //Ratio between the desired hip height and the roof height
  private static final float HIP_HEIGHT_RATIO = 0.5f;
  ...
  public HalfHippedRoof(float roofHeight, boolean alongLongSide) {
  ...
  public void calculateRoof(Geometry minRectangle) {
    ...
    // Rotate the roof by 90 degrees if requested
    ...
    halfCrossLength = e1VecLength / 2;
    
    // Calculate additonal middle points for half-hipps
    Vector e1Quarter1Point = c1Point.getPointBetween2D(e1MidPoint, HIP_HEIGHT_RATIO);
    Vector e1Quarter3Point = e1MidPoint.getPointBetween2D(c2Point, HIP_HEIGHT_RATIO);
    Vector e2Quarter1Point = c3Point.getPointBetween2D(e3MidPoint, HIP_HEIGHT_RATIO);
    Vector e2Quarter3Point = e3MidPoint.getPointBetween2D(c4Point, HIP_HEIGHT_RATIO);
    ...
    lineCoords[0] = new Coordinate(r1Point.x, r1Point.y);
    lineCoords[1] = new Coordinate(e1Quarter1Point.x, e1Quarter1Point.y);
    lines[1] = new LineString(new CoordinateArraySequence(lineCoords), geoFac);
    lineCoords = new Coordinate[2];
    lineCoords[0] = new Coordinate(r1Point.x, r1Point.y);
    lineCoords[1] = new Coordinate(e1Quarter3Point.x, e1Quarter3Point.y);
    lines[2] = new LineString(new CoordinateArraySequence(lineCoords), geoFac);
    lineCoords = new Coordinate[2];
    lineCoords[0] = new Coordinate(r2Point.x, r2Point.y);
    lineCoords[1] = new Coordinate(e2Quarter1Point.x, e2Quarter1Point.y);
    lines[3] = new LineString(new CoordinateArraySequence(lineCoords), geoFac);
    lineCoords = new Coordinate[2];
    lineCoords[0] = new Coordinate(r2Point.x, r2Point.y);
    lineCoords[1] = new Coordinate(e2Quarter3Point.x, e2Quarter3Point.y);
    lines[4] = new LineString(new CoordinateArraySequence(lineCoords), geoFac);
    
    // Half-hipped roof for a rectangle has 4 parts, calculate the first part
    polygons = new Polygon[4];
    Coordinate[] polyCoords = new Coordinate[7];
    polyCoords[0] = new Coordinate(c1Point.x, c1Point.y);
    polyCoords[1] = new Coordinate(e1Quarter1Point.x, e1Quarter1Point.y);
    polyCoords[2] = new Coordinate(r1Point.x, r1Point.y);
    polyCoords[3] = new Coordinate(r2Point.x, r2Point.y);
    polyCoords[4] = new Coordinate(e2Quarter3Point.x, e2Quarter3Point.y);
    polyCoords[5] = new Coordinate(c4Point.x, c4Point.y);
    polyCoords[6] = new Coordinate(polyCoords[0]);
    polygons[0] = new Polygon(new LinearRing(new CoordinateArraySequence(polyCoords), geoFac), null, geoFac);
    
    // Calculate the second part
    polyCoords = new Coordinate[7];
    polyCoords[0] = new Coordinate(r1Point.x, r1Point.y);
    polyCoords[1] = new Coordinate(e1Quarter3Point.x, e1Quarter3Point.y);
    polyCoords[2] = new Coordinate(c2Point.x, c2Point.y);
    polyCoords[3] = new Coordinate(c3Point.x, c3Point.y);
    polyCoords[4] = new Coordinate(e2Quarter1Point.x, e2Quarter1Point.y);
    polyCoords[5] = new Coordinate(r2Point.x, r2Point.y);
    polyCoords[6] = new Coordinate(polyCoords[0]);
    ...
    polyCoords[0] = new Coordinate(e1Quarter1Point.x, e1Quarter1Point.y);
    polyCoords[1] = new Coordinate(r1Point.x, r1Point.y);
    polyCoords[2] = new Coordinate(e1Quarter3Point.x, e1Quarter3Point.y);
    ...
    polyCoords = new Coordinate[4];
    polyCoords[0] = new Coordinate(e2Quarter1Point.x, e2Quarter1Point.y);
    polyCoords[1] = new Coordinate(r2Point.x, r2Point.y);
    polyCoords[2] = new Coordinate(e2Quarter3Point.x, e2Quarter3Point.y);
    polyCoords[3] = new Coordinate(polyCoords[0]);
  ...
  public double calculateRoofPointHeight(double x, double y) {
    ...
    if (tri1.contains(p)) {
      double distanceFromEdge = Math.abs((x - c1Point.x) * (e1Vec.y) - 
          (y - c1Point.y) * (e1Vec.x)) / e1VecLength;
      return  (0.5 + distanceFromEdge / triLength * 0.5) * roofHeight;
    } else if (tri2.contains(p)) {
      double distanceFromEdge = Math.abs((x - c3Point.x) * (e3Vec.y) - 
          (y - c3Point.y) * (e3Vec.x)) / e1VecLength;
      return  (0.5 + distanceFromEdge / triLength * 0.5) * roofHeight;
    }
    
    // If not, calculate point's height based on it's distance from the roof ridge
